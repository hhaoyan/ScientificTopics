...
class PunktToken(object):
    ...
    def __getstate__(self):
        data = (self.tok, self.type, self.period_final,
                self.parastart, self.linestart, self.sentbreak, self.abbr, self.ellipsis)
        return data

    def __setstate__(self, state):
        (self.tok, self.type, self.period_final,
         self.parastart, self.linestart, self.sentbreak, self.abbr, self.ellipsis) = state
...
class PunktBaseClass(object):
    ...
    def _tokenize_words(self, plaintext):
        ...
        plaintext is either a string representing the entire corpus, or a generator,
        which, when called, will generate a list of paragraphs.
        """
        parastart = False
        ...
        if isinstance(plaintext, string_types):
            ...
            paragraph_generator = gen_paragraphs()
        else:
            paragraph_generator = plaintext

        for line in paragraph_generator:
            if line.strip():
...
class PunktTrainer(PunktBaseClass):
    ...
    def train(self, text, verbose=False, finalize=True):
        ...
        text is either a string representing the entire corpus, or a generator,
        which, when called, will generate a list of paragraphs.
        """
    ...
    def _train_tokens(self, tokens, verbose, io_filename='tokens_cache.pickle'):
        self._finalized = False

        pickle_file = open(io_filename, 'wb')
        ...
        if verbose:
            logging.info('Counting type distribution...')
            tokens_iter = tqdm(tokens, desc='Tokenizing words', unit='tokens')
        else:
            tokens_iter = tokens

        for aug_tok in tokens_iter:
            self._type_fdist[aug_tok.type] += 1
            if aug_tok.period_final:
                self._num_period_toks += 1
            pickler.dump(aug_tok)
            pickler.clear_memo()

        pickle_file.close()

        tokens_count = tokens_iter.n

        # Look for new abbreviations, and for types that no longer are
        unique_types = set(self._type_fdist.keys())
        if verbose:
            logging.info('Looking for abbreviations (first pass)...')
            abbrev_type_iter = self._reclassify_abbrev_types(
                tqdm(unique_types, desc='Finding abbr. types',
                     unit='tokens', total=len(unique_types)))
        else:
            abbrev_type_iter = self._reclassify_abbrev_types(unique_types)

        for abbr, score, is_add in abbrev_type_iter:
            if score >= self.ABBREV:
                if is_add:
                    self._params.abbrev_types.add(abbr)
                    if verbose:
                        logging.info(('  Abbreviation: [%6.4f] %s' % (score, abbr)))
            else:
                if not is_add:
                    self._params.abbrev_types.remove(abbr)
                    if verbose:
                        logging.info(('  Removed abbreviation: [%6.4f] %s' % (score, abbr)))

        # Make a preliminary pass through the document, marking likely
        # sentence breaks, abbreviations, and ellipsis tokens.
        def read_object():
            with open(io_filename, 'rb') as unpickle_file:
                unpickler = Unpickler(unpickle_file)
                while True:
                    try:
        ...
        if verbose:
            logging.info('Annotating all tokens (first pass)...')
            tokens_iter = tqdm(
                self._annotate_first_pass(read_object()),
                desc='First annotation', unit='tokens', total=tokens_count)
        else:
            tokens_iter = self._annotate_first_pass(read_object())

        # We reuse the list, replacing old tokens with new ones to save memory
        pickle_file = open('.' + io_filename, 'wb')
        pickler = Pickler(pickle_file)

        for new_token in tokens_iter:
            pickler.dump(new_token)
            pickler.clear_memo()

        pickle_file.close()
        os.unlink(io_filename)
        os.rename('.' + io_filename, io_filename)

        # Check what contexts each word type can appear in, given the
        # case of its first letter.
        if verbose:
            logging.info('Obtaining orthography data...')
        ...
        def count_and_yield():
            ...
            self._sentbreak_count += count

        if verbose:
            tokens_iter = tqdm(
                count_and_yield(),
                desc='Processing orthography data', unit='tokens', total=tokens_count)
        else:
            tokens_iter = count_and_yield()
        self._get_orthography_data(tokens_iter)

        # We need total number of sentence breaks to find sentence starters
        # if verbose:
        #     print('Counting sentence breaks...')
        # self._sentbreak_count += self._get_sentbreak_count(tokens)

        # The remaining heuristics relate to pairs of tokens where the first
        # ends in a period.
        if verbose:
            logging.info('Using heuristics to detect additional abbreviations...')
            token_pair_iter = tqdm(
                _pair_iter(read_object()),
                desc='Searching for rare abbr.', unit='tokens', total=tokens_count)
        else:
            token_pair_iter = _pair_iter(read_object())

        for aug_tok1, aug_tok2 in token_pair_iter:
            ...
            if self._is_rare_abbrev_type(aug_tok1, aug_tok2):
                ...
                if verbose:
                    logging.info(('  Rare Abbrev: %s' % aug_tok1.type))
